...
<?php 
  function nama_bantuan_terpilih($id_penduduk, $id_jenis_bantuan){
    include "../../assets/koneksi.php";
    $cek_bantuan_didaftarkan = mysqli_query($conn, "SELECT * from alternatif a left join jenis_bantuan jb on a.id_jenis_bantuan=jb.id_jenis_bantuan where a.id_jenis_bantuan='$id_jenis_bantuan' and a.id_penduduk='$id_penduduk'");
    $d_bantuan_didaftarkan = mysqli_fetch_array($cek_bantuan_didaftarkan);
    $nama_jenis_bantuan_didaftarkan = $d_bantuan_didaftarkan['nama_jenis_bantuan'];
    return $nama_jenis_bantuan_didaftarkan;
  }
 ?>
	<div class="box-header with-border">
	  ...
	  <div class="pull-right">
           ...
           if ($_SESSION['level']=='Penduduk') { 
            if ($djb['status']=='Penetapan') { 
              $id_penduduk = $_SESSION['id_user'];
              $cek_alternatif = mysqli_query($conn, "SELECT * from alternatif where id_penduduk='$id_penduduk'");
              $j_cek_alternatif=mysqli_num_rows($cek_alternatif);
              $d_cek_alternatif = mysqli_fetch_array($cek_alternatif);
              $id_jenis_bantuan_didaftarkan = $d_cek_alternatif['id_jenis_bantuan'];
              
              if ($j_cek_alternatif>0) {
                $nama_jenis_bantuan_didaftarkan = nama_bantuan_terpilih($id_penduduk, $id_jenis_bantuan_didaftarkan);
                echo "Anda sudah melakukan pendaftaran sebagai calon penerima bantuan ".$nama_jenis_bantuan_didaftarkan;
              }else{
              ?>
            <a href="?a=alternatif_dipilih&id_penduduk=<?php echo $_SESSION['id_user'] ?>&id_jenis_bantuan=<?php echo $id ?>" class="btn btn-info btn-xs">Daftar Sebagai Calon Penerima Bantuan</a>
              <?php }
            }
            else{ ?>
              Jenis Bantuan Sudah Dipublikasi <br>
            <a href="?a=daftar_penerima_bantuan&id=<?php echo $djb['id_jenis_bantuan'] ?>" class="btn btn-info btn-xs">Lihat Laporan</a>
            <?php } ?>
      ...
      <a href="?a=jenis_bantuan" class="btn btn-primary btn-xs">Kembali</a> 
    <?php } ?>
        ...
        <?php }else{ ?>
			 <table class="table">
			 	<tr>
			 		...
			 		<td>Kategori</td>
			 		<td>Bobot</td>
          <td <?php if ($_SESSION['level']!='Admin') {echo "style='display:none'";} ?>>Pilihan Alternatif</td>
          <td <?php if ($_SESSION['level']!='Admin') {echo "style='display:none'";} ?>>Option</td>
			 	</tr>
			<?php 
          ...
          <td  <?php if ($_SESSION['level']!='Admin') {echo "style='display:none'";} ?>>
            <ol>
...
?>
	<tr <?php if ($_SESSION['level']=='Penduduk' && $_SESSION['id_user']==$d_alternatif['id_penduduk']) {echo "style='background: #d2fefc '";} ?>>
	...
	<td>
   <?php if ($_SESSION['level']=='Penduduk' && $_SESSION['id_user']==$d_alternatif['id_penduduk']) { ?>
